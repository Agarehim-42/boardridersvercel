...
const PaymentComponent = () => {
  ...
  // Valyuta forması üçün state
  const [currency, setCurrency] = useState('usd'); // Əsasən usd istifadə olunur

  // Səbətdəki məhsulların cəmi məbləğini hesablamaq
  ...
  // Ödəniş intenti yaratmaq üçün formun göndərilməsi
  const handleSubmit = (e) => {
    e.preventDefault();
    // Diqqət: Məbləğ ən kiçik valyuta vahidi (məsələn, cent/kuruş) şəklində verilməlidir.
  ...
  // PaymentIntent uğurlu yarandıqdan sonra form məlumatlarını sıfırlamaq üçün useEffect
  useEffect(() => {
    if (clientSecret) {
      // 3 saniyə sonra sıfırlamaq (bu müddəti öz ehtiyacınıza görə tənzimləyə bilərsiniz)
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      <h2 className="text-3xl font-bold text-center mb-6">Ödəniş Səhifəsi</h2>
          ...
          <div className="space-y-4">
            ...
            <div className="mt-4 flex justify-end">
              <p className="text-xl font-bold">
                Ümumi: {totalAmount.toFixed(2)}₼
              </p>
            </div>
          </div>
        ) : (
          <p>Səbətiniz boşdur.</p>
        )}
      </div>

      {/* Ödəniş Formu */}
      <div className="mb-8">
        <h3 className="text-2xl font-semibold mb-4">Ödəniş</h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex space-x-4">
            ...
            >
              {loading ? 'Yüklənir...' : 'Ödənişi Başlat'}
            </button>
            <button
              type="button"
              onClick={handleReset}
      ...
      {clientSecret && (
        <div className="mt-6 p-4 border border-green-300 bg-green-50 rounded">
          ...
          {/* Gələcəkdə burada Stripe Elements və ya digər ödəniş vasitələri inteqrasiyası üçün əlavə payment kodları əlavə olunacaq */}
        </div>
      )}
